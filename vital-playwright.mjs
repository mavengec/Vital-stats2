/**
 * Vital Rust → Discord трекер (версія зі справжнім браузером).
 * Запускає headless Chromium (Playwright) у хмарі GitHub Actions — 24/7, без ПК.
 * Саме справжній браузер потрібен тому, що API сайту віддає статистику тільки
 * запитам із браузера (з серверів напряму повертає 500).
 *
 * Налаштування — змінні середовища (Secrets/Variables репозиторію):
 *   DISCORD_WEBHOOK_URL (обовʼязково), THREAD_ID, STEAM_IDS, SERVER_IDS, MODE, TITLE
 * ID повідомлення Discord зберігається у state.json (його комітить workflow).
 */
...
const SITE = "https://vitalrust.com/statistics";
...
// ───────────────────────── отримання даних через справжній браузер ─────────────────────────
async function fetchData(cfg) {
  ...
  try {
    ...
    // відкриваємо сайт, щоб отримати контекст справжнього браузера
    await page.goto(SITE, { waitUntil: "domcontentloaded", timeout: 60000 });
    await page.waitForTimeout(2500);

    // робимо запити до API прямо в контексті сторінки (як це робить сам сайт)
    const raw = await page.evaluate(async ({ api, serverIds, steamIds }) => {
      const wanted = new Set(steamIds.map(String));
      const out = {};
      for (const id of serverIds) {
        try {
          const wj = await (await fetch(`${api}/servers/${id}/wipes`, { headers: { accept: "application/json" } })).json();
          const wipes = wj.data || wj;
          const open = wipes.find((w) => !w.endTime) || wipes[0];
          const r = await fetch(`${api}/players/overview`, {
            method: "POST",
            headers: { "content-type": "application/json", accept: "application/json" },
            body: JSON.stringify({ serverId: id, wipeId: open.id }),
          });
          if (!r.ok) { out[id] = { error: `overview ${r.status}` }; continue; }
          const j = await r.json();
          const arr = j.data || j.players || j.items || j;
          const matched = (Array.isArray(arr) ? arr : []).filter((p) => {
            const ids = JSON.stringify(p).match(/7656\d{13}/g) || [];
            return ids.some((x) => wanted.has(x));
          });
          out[id] = { wipeId: open.id, matched };
        } catch (e) { out[id] = { error: String(e) }; }
      }
      return out;
    }, { api: STATS_API, serverIds: cfg.SERVER_IDS, steamIds: cfg.STEAM_IDS });
...
async function main() {
  ...
  const raw = await fetchData(cfg);

  // перевірка помилок по серверах
  const errors = [];
  for (const id of cfg.SERVER_IDS) {
    if (raw[id] && raw[id].error) errors.push(`сервер ${SERVER_NAMES[id] || id}: ${raw[id].error}`);
  }
  if (errors.length === cfg.SERVER_IDS.length) {
    throw new Error("Не вдалося отримати дані з жодного сервера → " + errors.join(" | "));
  }

  const rows = [];
  for (const id of cfg.SERVER_IDS) {
    const block = raw[id];
    if (!block || !block.matched) continue;
    const byId = indexBySteamId(block.matched);
    for (const sid of cfg.STEAM_IDS) {
      const p = byId.get(String(sid));
      if (!p) continue;
      rows.push({ serverId: id, sid: String(sid), name: pickName(p, sid), stats: extractStats(p) });
    }
  }
  ...
  const content = renderMessage(cfg, tableRows, errors);
  await upsertDiscordMessage(cfg, content);
  console.log("OK, гравців у повідомленні:", tableRows.length, errors.length ? "| попередження: " + errors.join("; ") : "");
}
...
function renderMessage(cfg, rows, errors = []) {
